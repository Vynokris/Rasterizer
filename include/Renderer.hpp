...
public:
    ...
    void drawPixel        (const unsigned int& _x,     const unsigned int& _y,
                           const float&        _depth, const Color& _color);

    void drawLine         (const geometry3D::Vertex& _p0, const geometry3D::Vertex& _p1);

    bool transformVertices(int      _count, Vertex*  _vertices, Vector3* _local, 
                           Vector4* _world, Vector4* _view,     Vector4* _clip,
                           Vector3* _ndc,   Vector3* _screen,   Vector3* _perspectiveUV);

    void drawTriangle     (geometry3D::Triangle3 _triangle, TextureData* _texture);

    void drawTriangles    (geometry3D::Triangle3* _triangles, const unsigned int& _count,
                           TextureData* _texture);

    void drawDividedQuad  (const Color& _color, const float& _size = 1.f,
                           const bool&  _negateNormals = false);
    
    //! Using meshes instead.
    /*
    void drawCube         (const Color& _color, const float& _size = 1.f);

    void drawSphere       (const float& _r, const int& _lon, const int& _lat,
                           const Color& _color);
    */
